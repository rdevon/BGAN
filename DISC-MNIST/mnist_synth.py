...
def train(num_epochs=200, n_samples=20, initial_eta=1e-4, plot_colour="-b",
          source=None, out_path='./'):
    if source is None:
        raise ValueError('source not provided.')
    gen_dir = path.join(out_path, 'generated_images')
    gt_dir = path.join(out_path, 'gt_images')
    bin_dir = path.join(out_path, 'checkpoints')

    if not path.isdir(gen_dir):
        os.mkdir(gen_dir)

    if not path.isdir(gt_dir):
        os.mkdir(gt_dir)
    ...
    for epoch in range(num_epochs):
        ...
        pbar = ProgressBar(widgets=widgets, maxval=(50000//128)).start()
        for i, batch in enumerate(iterate_minibatches(X_train, 128, shuffle=True)):
        ...
        if epoch % 1 == 0:
            ...
            plt.imsave(path.join(gen_dir, prefix + '.png'),
        ...
        if epoch % 10 == 0:
            ...
            np.savez(path.join(bin_dir, prefix + '_disc_mnist_gen_params.npz'),
                     *lasagne.layers.get_all_param_values(generator))
            np.savez(path.join(bin_dir, prefix + '_disc_mnist_disc_params.npz'),
...
if __name__ == '__main__':
    if ('--help' in sys.argv) or ('-h' in sys.argv):
        ...
        for eta in eta_array:
            ...
            train(initial_eta=eta, source=SOURCE, out_path=OUT_PATH)
