...
def BGAN2(fake_out, real_out, Z):
    '''No nonlinearity on discriminator. LSGAN-like loss.
    
    '''
    w = fake_out / (1. + fake_out)
    
    log_N = T.log(w.shape[0]).astype(w.dtype)
    Z_est = T.sqrt(w ** 2).mean(0)
    Z_est = theano.gradient.disconnected_grad(Z_est)
         
    generator_loss = ((w - 0.5) ** 2).mean()
    #generator_loss = ((w - Z / (1. + Z)) ** 2).mean()
    discriminator_loss = ((real_out - 1.) ** 2).mean() + (fake_out ** 2).mean()
    return generator_loss, discriminator_loss, Z_est
...
def train(num_epochs,
    ...
    log_Z = theano.shared(lasagne.utils.floatX(1.), name='log_Z')
    ...
    generator_loss, discriminator_loss, log_Z_est = BGAN2(fake_out, real_out, log_Z) 
